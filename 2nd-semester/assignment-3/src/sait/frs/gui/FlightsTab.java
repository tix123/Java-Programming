...
public class FlightsTab extends TabBase
{
	...
	private JPanel finderCenterPanel()
	{
		...
		panel.setLayout(new GridLayout(0, 1));
		String[] weekdayList =
		{ Manager.WEEKDAY_ANY, Manager.WEEKDAY_MONDAY, Manager.WEEKDAY_TUESDAY, Manager.WEEKDAY_WEDNESDAY, Manager.WEEKDAY_THURSDAY, Manager.WEEKDAY_FRIDAY, Manager.WEEKDAY_SATURDAY, Manager.WEEKDAY_SUNDAY };

		fromBox = new JComboBox(manager.getAirports().toArray());
		panel.add(fromBox);

		toBox = new JComboBox(manager.getAirports().toArray());
		panel.add(toBox);
	...
	private class ReserveButtonListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			try 
			{
				Reservation reservation = manager.makeReservation(flightsList.getSelectedValue(), nameText.getText(), citizenshipText.getText());
	...
	private class FlightFinderButtonListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			ArrayList<Flight> flights = manager.findFlights((String) fromBox.getSelectedItem(), 
